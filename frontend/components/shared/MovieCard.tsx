...
export function MovieCard({ movie }: { movie: MovieSummary }) {
  return <Link className="movie-card film-card" href={`/movies/${movie.slug}`}>
    ...
    <div className="movie-card-body">
      <div className="film-card-meta"><span>{movie.release_date.slice(0, 4)}</span><span>{movie.imdb_rating ? `IMDb ${movie.imdb_rating}` : movie.status === 'upcoming' ? 'Coming soon' : 'Released'}</span></div>
      <h3>{movie.title}</h3>
