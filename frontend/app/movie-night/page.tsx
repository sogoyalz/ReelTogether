...
type Room = { id:string; title:string; state:string; round_id:number; is_host:boolean; expired:boolean; expires_at:string; winner_id:number|null; members:{account_id:number;username:string;is_host:boolean;complete:boolean}[]; preferences:{genres:string[];favorites:number[]}; favorite_movies:{id:number;title:string}[]; movies:Film[]; my_votes:Record<string,string>; results:Result[] }
...
function NightWorkspace({auth}:{auth:AuthState|null}){
  ...
  async function act(path:string,payload:unknown={},method:'post'|'put'='post'){
    ...
    catch(e){setError(accountError(e));if(isAxiosError(e)&&e.response?.status===409)await client.invalidateQueries({queryKey:['movie-night']})}finally{setBusy(false)}
  }
    ...
    {!auth?<section className="panel"><h2>Bring your people together</h2><p>Sign in to create or join a room. If you have an invitation, return to this link after signing in.</p><Link className="cta-button" href="/watchlist">Sign in to join</Link></section>:<>
      ...
      {roomId&&<>
        ...
        {data.state==='lobby'&&<>
          ...
          <PreferencesEditor key={`${roomId}:${data.round_id}`} data={data} busy={busy} onSave={async p=>{const result=await act(`/api/movie-nights/${roomId}/preferences`,p,'put');if(result)setNotice('Your preferences are saved.');return !!result}} onStart={async()=>{await act(`/api/movie-nights/${roomId}/start`)}}/>
          {data.is_host&&<section className="panel"><h3>Manage your group</h3><p className="meta">Remove anyone who cannot join this round. Removing a member also replaces the invite link.</p>{data.members.filter(m=>!m.is_host).map(m=><button className="cta-button secondary-button" key={m.account_id} disabled={busy} onClick={async()=>{const result=await act(`/api/movie-nights/${roomId}/remove-member`,{account_id:m.account_id});if(result)shareToken(result.invite_token)}}>Remove {m.username} from room</button>)}</section>}
        </>}
        {data.state==='voting'&&<section><h2>Your private ballot</h2><p className="subtle">Yes means you’d watch it. Pass is neutral. A single veto excludes a film. Choose one on every card; you can change votes until the reveal.</p><div className="night-films">{data.movies.map(movie=><article className="night-film" key={movie.id}><Link href={`/movies/${movie.slug}`}><MovieArtwork src={movie.poster_url} title={movie.title}/><h3>{movie.title}</h3></Link><p className="meta">{movie.year} · {movie.genres.slice(0,2).join(' / ')}</p><div className="night-votes" role="group" aria-label={`Vote on ${movie.title}`}>{([{value:'yes',label:'Yes',icon:Check},{value:'pass',label:'Pass',icon:Minus},{value:'veto',label:'Veto',icon:Ban}] as const).map(({value,label,icon:Icon})=><button key={value} aria-pressed={data.my_votes[movie.id]===value} disabled={frozen} onClick={()=>act(`/api/movie-nights/${roomId}/vote`,{movie_id:movie.id,choice:value,round_id:data.round_id},'put')}><Icon size={14}/>{label}</button>)}</div></article>)}</div><div className="night-stage-action"><p role="status">{data.members.filter(m=>m.complete).length} of {data.members.length} ballots complete. Only completion status is shared.</p>{data.is_host&&<button className="cta-button" disabled={frozen||!complete} onClick={()=>act(`/api/movie-nights/${roomId}/reveal`)}>Reveal group matches</button>}</div></section>}
        </>}
        {!data.expired&&data.is_host&&['voting','revealed'].includes(data.state)&&<section className="panel"><h3>Someone cannot finish?</h3><p className="meta">Return to the lobby to adjust the group and preferences. This clears all ballots and replaces the invitation. Everyone votes again.</p><button className="cta-button secondary-button" disabled={busy} onClick={async()=>{const result=await act(`/api/movie-nights/${roomId}/restart`,{round_id:data.round_id});if(result){shareToken(result.invite_token);setNotice('Round cleared. Adjust your group, then start again.')}}}>Return to lobby and clear votes</button></section>}
        {(data.state==='revealed'||data.state==='selected')&&<section><h2>{data.state==='selected'?'Tonight’s pick':'Your group matches'}</h2><p className="subtle">Most yes votes wins among films with no veto. Equal scores are tied; the host chooses between top matches.</p>{data.results.every(r=>!r.eligible)&&<p role="status">Every film was vetoed. There is no winner—ask the host to return to the lobby and adjust the group’s preferences.</p>}<div className="night-films">{data.results.filter(r=>data.state!=='selected'||r.movie_id===data.winner_id).map(result=>{const movie=data.movies.find(m=>m.id===result.movie_id);return movie&&<article className="night-film" key={movie.id}><Link href={`/movies/${movie.slug}`}><MovieArtwork src={movie.poster_url} title={movie.title}/><h3>{movie.title}</h3></Link>{result.top_match&&<span className="eyebrow">Top match</span>}<p className="meta">{result.reason}</p>{data.is_host&&data.state==='revealed'&&result.top_match&&<button className="cta-button" disabled={frozen} onClick={()=>act(`/api/movie-nights/${roomId}/select`,{movie_id:movie.id})}>Choose {movie.title}</button>}{data.state==='selected'&&<><WatchlistButton movieId={movie.id}/><p className="meta">Optional. Saves to your watchlist only.</p></>}</article>})}</div></section>}
        </>}
      </>}
    </>}
  </main>
}
function PreferencesEditor({data,busy,onSave,onStart}:{data:Room;busy:boolean;onSave:(p:{genres:string[];favorites:number[]})=>Promise<boolean>;onStart:()=>Promise<void>}){
  ...
  const dirty=JSON.stringify([...selected].sort())!==JSON.stringify([...data.preferences.genres].sort())||JSON.stringify(favorites.map(f=>f.id).sort((a,b)=>a-b))!==JSON.stringify([...data.preferences.favorites].sort((a,b)=>a-b))
  async function save(start=false){
    if(saving||busy)return
    setSaving(true)
    try{if(await onSave({genres:selected,favorites:favorites.map(f=>f.id)})){if(start)await onStart()}}finally{setSaving(false)}
  }
