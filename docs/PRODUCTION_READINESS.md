...
- **Verified scheduled backups:** the Compose `backups` service starts after backend readiness, creates an online SQLite backup daily, restores it into a temporary file, compares complete contents, and publishes success only afterward. It retains the latest 14 worker-created backups and retries failures after five minutes. A health check detects overdue success. This is a local-volume backup, not offsite disaster recovery; it cannot survive loss of the whole volume.
